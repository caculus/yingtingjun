...
- 首次啟動請預留約 **6–8 GB** 磁碟空間（Python、套件、Whisper、翻譯與話者模型、ECDICT 詞典；精簡安裝包會自動下載）
...
歡迎卡可以打開內建 **試用教材**（Aesop 公版寓言）、從 YouTube 開始、匯入本機錄音，或播放新手導引。之後可點右上角 **？** 重播導引。

播放器預設為 **學習模式**（隱藏工程向控件）。標題旁打開 **進階** 後，才看得到局部重辨、重新命名、匯出文稿、筆記 CSV 匯出等。

試用教材只會複製到「文件」學習資料一次。若你刪掉它，程式不會自動再種一份；可從歡迎卡再要求打開（強制複製）。
