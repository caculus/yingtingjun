...
1. 第一次打開播放器會看到歡迎卡：可先開內建試用教材，或從 YouTube／本機錄音開始
2. 匯入任意英文對話錄音，或從播放器 **匯入 → YouTube** 貼上影片連結
3. 偵測語言、轉寫英文、加上中文翻譯（YouTube 有中文字幕時會優先採用；有英文字幕時可走字幕快徑）
4. 在瀏覽器以**學習模式**邊聽邊跟讀、查詞、做筆記；需要時再開「進階」做局部重辨或匯出
...
- 本機轉寫與翻譯
- **YouTube 匯入**（內建；開發環境需 `pip install -r requirements-youtube.txt`；安裝包首次啟動會自動安裝 `yt-dlp`；有中文字幕時優先採用；英文字幕路徑預設會用 Whisper **對齊光棒時間**、保留字幕文字）
- 話者標記、時間戳、詞級時間
- 瀏覽器同步播放與跟讀，支援 0.5×–2.0× 播放速度
- **學習模式**（預設隱藏工程向控件；可開「進階」）與 **新手導引**
- 內建公版 **試用教材**（Aesop 寓言，首次啟動可直接打開）
- ECDICT 優先的點詞查詢
- 每支錄音獨立的學習筆記與 CSV 匯出
- **匯出雙語文稿**為 `.txt`／`.md`／`.html`（先開「進階」，再 **打開 ▾ → 匯出**；可選說話者／時間戳）
- 只重跑局部區段的局部重辨
...
安裝後瀏覽器會開 `http://127.0.0.1:8765/`。歡迎卡可：

- **打開試用教材**（Aesop 公版寓言，不必先轉寫）
- **從 YouTube 開始** 或 **匯入錄音**
- **看使用說明**（之後也可點右上角「？」重播；已打開文稿時會改播「文稿說明」）

預設是學習模式（隱藏局部重辨、匯出 CSV 等工程向控件）。需要那些功能時，打開標題旁的 **進階**。

### 從 YouTube 匯入（選用）

播放器內 **匯入 ▾ → YouTube…**（選單第一項；本機檔為 **本機錄音檔…**）：貼 URL、可改教材名稱，完成後自動載入雙語文稿。有中文字幕時優先用影片字幕，不必再跑本機翻譯；英文字幕路徑預設會用 Whisper **對齊光棒時間**。  
